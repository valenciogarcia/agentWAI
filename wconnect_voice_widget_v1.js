(() => {
  if (document.getElementById("wconnect-voice-widget")) return;

  const scriptElement = document.currentScript;

  const normalizePublicId = value => {
    const normalized = String(value ?? "").trim();

    if (!/^\d+$/.test(normalized)) return "";
    return Number(normalized) > 0 ? normalized : "";
  };

  const WCONNECT_ACCOUNT_ID = normalizePublicId(
    scriptElement?.dataset.accountId
  );

  const WCONNECT_INBOX_ID = normalizePublicId(
    scriptElement?.dataset.inboxId
  );

  if (!WCONNECT_ACCOUNT_ID || !WCONNECT_INBOX_ID) {
    console.error(
      "[WCONNECT VOICE] data-account-id e data-inbox-id são obrigatórios."
    );
    return;
  }

  const WCONNECT_VISITOR_NAMESPACE =
    `chatwoot:${WCONNECT_ACCOUNT_ID}:inbox:${WCONNECT_INBOX_ID}`;

  const style = document.createElement("style");
  style.textContent = `
    #wconnect-voice-widget {
      position: fixed;
      top: 50%;
      right: 30px;
      left: auto;
      transform: translateY(-50%);
      width: 220px;
      height: 97px;
      background: #1b1b34;
      border-radius: 14px;
      box-shadow: 0 12px 28px rgba(0,0,0,0.35);
      z-index: 999999;
      overflow: hidden;
      border: 1px solid rgba(255,255,255,0.05);
      touch-action: none;
      color: white;
      font-family: Arial, sans-serif;
    }

    #wconnect-voice-widget * {
      box-sizing: border-box;
    }

    #wconnect-widget-body {
      padding: 6px 10px 8px;
    }

    #wconnect-voice-widget .title {
      font-size: 16px;
      font-weight: 700;
      line-height: 1.08;
      margin: 0 0 8px;
      color: #f4f4f5;
      text-align: center;
    }

    #wconnect-voice-widget .action-area {
      display: flex;
      justify-content: center;
    }

    #wconnect-voice-widget button.actionBtn {
      width: 180px;
      max-width: 100%;
      min-height: 38px;
      border: none;
      border-radius: 12px;
      font-size: 11px;
      font-weight: 700;
      cursor: pointer;
    }

    #wconnect-btnLigar {
      background: #53b44f;
      color: white;
      display: block;
    }

    #wconnect-btnDesligar {
      background: #e54848;
      color: white;
      display: none;
    }

    #wconnect-status {
      margin-top: 6px;
      text-align: center;
      font-size: 9px;
      color: rgba(255,255,255,0.78);
      min-height: 14px;
      word-break: break-word;
    }
  `;
  document.head.appendChild(style);

  const widget = document.createElement("div");
  widget.id = "wconnect-voice-widget";
  widget.innerHTML = `
    <div id="wconnect-widget-body">
      <h1 class="title">🗣️ Tire suas dúvidas</h1>
      <div class="action-area">
        <button id="wconnect-btnLigar" class="actionBtn">🎤 Converse com nosso Agente</button>
        <button id="wconnect-btnDesligar" class="actionBtn">⏹️ Desligar</button>
      </div>
      <div id="wconnect-status">Aguardando...</div>
    </div>
  `;
  document.body.appendChild(widget);

  const btnLigar = document.getElementById("wconnect-btnLigar");
  const btnDesligar = document.getElementById("wconnect-btnDesligar");
  const statusDiv = document.getElementById("wconnect-status");

  let session = null;
  let lastClickedElement = null;

  function setStatus(text) {
    statusDiv.innerText = text;
    console.log("[STATUS]", text);
  }

  async function sha256(text) {
    const encoder = new TextEncoder();
    const data = encoder.encode(text);
    const hashBuffer = await crypto.subtle.digest("SHA-256", data);

    return Array.from(new Uint8Array(hashBuffer))
      .map(byte => byte.toString(16).padStart(2, "0"))
      .join("");
  }

  function normalizeText(value) {
    return String(value || "")
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/\s+/g, " ")
      .trim();
  }

  function bucketNumber(value, step = 100) {
    const n = Number(value || 0);
    if (!n || Number.isNaN(n)) return null;
    return Math.round(n / step) * step;
  }

  function getApproxOS() {
    const ua = navigator.userAgent || "";
    const platform = navigator.platform || "";
    const uaDataPlatform = navigator.userAgentData?.platform || "";

    const text = `${ua} ${platform} ${uaDataPlatform}`.toLowerCase();

    if (text.includes("mac")) return "macos";
    if (text.includes("win")) return "windows";
    if (text.includes("android")) return "android";
    if (text.includes("iphone") || text.includes("ipad") || text.includes("ipod")) return "ios";
    if (text.includes("linux")) return "linux";

    return "unknown";
  }

  function getGpuFamily(value) {
    const text = normalizeText(value);

    if (!text) return "unknown";
    if (text.includes("apple")) return "apple";
    if (text.includes("nvidia") || text.includes("geforce") || text.includes("rtx") || text.includes("gtx")) return "nvidia";
    if (text.includes("amd") || text.includes("radeon")) return "amd";
    if (text.includes("intel")) return "intel";
    if (text.includes("adreno")) return "adreno";
    if (text.includes("mali")) return "mali";
    if (text.includes("powervr")) return "powervr";

    return "other";
  }

  function getWebGLStableInfo() {
    try {
      const canvas = document.createElement("canvas");
      const gl =
        canvas.getContext("webgl") ||
        canvas.getContext("experimental-webgl");

      if (!gl) {
        return {
          vendorFamily: "unknown",
          rendererFamily: "unknown"
        };
      }

      const debugInfo = gl.getExtension("WEBGL_debug_renderer_info");

      const vendor = debugInfo
        ? gl.getParameter(debugInfo.UNMASKED_VENDOR_WEBGL)
        : gl.getParameter(gl.VENDOR);

      const renderer = debugInfo
        ? gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL)
        : gl.getParameter(gl.RENDERER);

      return {
        vendorFamily: getGpuFamily(vendor),
        rendererFamily: getGpuFamily(renderer)
      };
    } catch {
      return {
        vendorFamily: "unknown",
        rendererFamily: "unknown"
      };
    }
  }

  function getCoresBucket() {
    const cores = Number(navigator.hardwareConcurrency || 0);

    if (!cores) return "unknown";
    if (cores <= 2) return "1-2";
    if (cores <= 4) return "3-4";
    if (cores <= 8) return "5-8";
    if (cores <= 16) return "9-16";

    return "17+";
  }

  function getStableScreenProfile() {
    const width = window.screen.width || null;
    const height = window.screen.height || null;

    const minSide = width && height ? Math.min(width, height) : null;
    const maxSide = width && height ? Math.max(width, height) : null;

    return {
      minSideBucket: bucketNumber(minSide, 100),
      maxSideBucket: bucketNumber(maxSide, 100),
      colorDepth: window.screen.colorDepth || null,
      pixelDepth: window.screen.pixelDepth || null
    };
  }

  function getStableDeviceProfile() {
    const gpu = getWebGLStableInfo();

    return {
      v: "wconnect-device-visitor-v3",

      // Namespace estável por conta e inbox do Chatwoot.
      namespace: WCONNECT_VISITOR_NAMESPACE,

      os: getApproxOS(),

      // Estável na maioria dos casos para o mesmo computador/local.
      timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || "",

      // Tela em buckets para reduzir diferenças entre browsers, zoom e escala.
      screen: getStableScreenProfile(),

      hardware: {
        coresBucket: getCoresBucket(),
        touchPoints: navigator.maxTouchPoints || 0
      },

      // Família da GPU, não string completa, para reduzir variação entre browsers.
      gpu: {
        vendorFamily: gpu.vendorFamily || "unknown",
        rendererFamily: gpu.rendererFamily || "unknown"
      }
    };
  }

  function getBrowserFallbackId() {
    const key = "wai_visitor_id_fallback_v2";
    let visitorId = "";

    try {
      visitorId = localStorage.getItem(key) || "";
    } catch {}

    if (!visitorId) {
      visitorId =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `fallback-${Date.now()}-${Math.random().toString(16).slice(2)}`;

      try {
        localStorage.setItem(key, visitorId);
      } catch {}
    }

    return `${WCONNECT_VISITOR_NAMESPACE}:browser:${visitorId}`;
  }

  /**
   * Gera dois identificadores:
   *
   * deviceVisitorId:
   * - tentativa determinística de reconhecer o mesmo computador;
   * - pode ser igual entre browsers se os sinais forem parecidos;
   * - não é 100% garantido por limitações de privacidade dos navegadores.
   *
   * browserVisitorId:
   * - persistente no mesmo navegador via localStorage;
   * - mais confiável dentro do mesmo browser;
   * - não atravessa Chrome/Safari/Firefox/Edge.
   */
  async function getVisitorIds() {
    try {
      const profile = getStableDeviceProfile();
      const hash = await sha256(JSON.stringify(profile));

      return {
        deviceVisitorId: `${WCONNECT_VISITOR_NAMESPACE}:device:${hash}`,
        browserVisitorId: getBrowserFallbackId(),
        deviceProfile: profile
      };
    } catch (error) {
      const browserVisitorId = getBrowserFallbackId();

      return {
        deviceVisitorId: browserVisitorId,
        browserVisitorId,
        deviceProfile: null
      };
    }
  }

  function podeArrastar(target) {
    return !target.closest("button");
  }

  let isDragging = false;
  let offsetX = 0;
  let offsetY = 0;

  function iniciarArraste(clientX, clientY) {
    const rect = widget.getBoundingClientRect();
    isDragging = true;
    offsetX = clientX - rect.left;
    offsetY = clientY - rect.top;
    widget.style.left = rect.left + "px";
    widget.style.top = rect.top + "px";
    widget.style.right = "auto";
    widget.style.transform = "none";
    document.body.style.userSelect = "none";
  }

  function moverWidget(clientX, clientY) {
    if (!isDragging) return;

    let left = clientX - offsetX;
    let top = clientY - offsetY;

    const maxLeft = window.innerWidth - widget.offsetWidth;
    const maxTop = window.innerHeight - widget.offsetHeight;

    if (left < 0) left = 0;
    if (top < 0) top = 0;
    if (left > maxLeft) left = maxLeft;
    if (top > maxTop) top = maxTop;

    widget.style.left = left + "px";
    widget.style.top = top + "px";
  }

  function finalizarArraste() {
    isDragging = false;
    document.body.style.userSelect = "auto";
  }

  widget.addEventListener("mousedown", function (e) {
    if (!podeArrastar(e.target)) return;
    iniciarArraste(e.clientX, e.clientY);
  });

  document.addEventListener("mousemove", function (e) {
    moverWidget(e.clientX, e.clientY);
  });

  document.addEventListener("mouseup", function () {
    finalizarArraste();
  });

  widget.addEventListener("touchstart", function (e) {
    if (!podeArrastar(e.target)) return;
    const touch = e.touches[0];
    iniciarArraste(touch.clientX, touch.clientY);
  }, { passive: true });

  document.addEventListener("touchmove", function (e) {
    const touch = e.touches[0];
    moverWidget(touch.clientX, touch.clientY);
  }, { passive: true });

  document.addEventListener("touchend", function () {
    finalizarArraste();
  });

  document.addEventListener("click", function (e) {
    const el = e.target.closest("button, a, input, select, textarea, label, [data-section], [data-product], [data-plan], [data-service], [id]");
    if (!el) return;

    if (el.id === "wconnect-btnLigar" || el.id === "wconnect-btnDesligar") return;

    lastClickedElement = {
      tag: el.tagName || "",
      id: el.id || "",
      className: typeof el.className === "string" ? el.className : "",
      text: (el.innerText || el.value || el.getAttribute("aria-label") || el.getAttribute("title") || "").trim().slice(0, 200),
      href: el.href || "",
      name: el.getAttribute("name") || "",
      type: el.getAttribute("type") || "",
      role: el.getAttribute("role") || "",
      dataSection: el.dataset?.section || "",
      dataProduct: el.dataset?.product || "",
      dataPlan: el.dataset?.plan || "",
      dataService: el.dataset?.service || ""
    };
  }, true);

  async function prepararMicrofone() {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      stream.getTracks().forEach(track => track.stop());
      return true;
    } catch (error) {
      setStatus("❌ Erro no microfone: " + error.message);
      return false;
    }
  }

  async function getPermissionState(name) {
    try {
      if (!navigator.permissions?.query) return "unsupported";
      const result = await navigator.permissions.query({ name });
      return result.state || "unknown";
    } catch {
      return "unsupported";
    }
  }

  async function getMediaInfo() {
    try {
      if (!navigator.mediaDevices?.enumerateDevices) return [];
      const devices = await navigator.mediaDevices.enumerateDevices();

      return devices.map(d => ({
        kind: d.kind,
        label: d.label || "",
        deviceId: d.deviceId ? "present" : "",
        groupId: d.groupId ? "present" : ""
      }));
    } catch (error) {
      return [{ error: error.message }];
    }
  }

  function getBrowserInfo() {
    return {
      url: window.location.href,
      origin: window.location.origin,
      pathname: window.location.pathname,
      referrer: document.referrer || "",
      title: document.title,
      userAgent: navigator.userAgent,
      language: navigator.language,
      languages: navigator.languages || [],
      platform: navigator.platform || "",
      cookieEnabled: navigator.cookieEnabled,
      onLine: navigator.onLine,
      hardwareConcurrency: navigator.hardwareConcurrency || null,
      deviceMemory: navigator.deviceMemory || null,
      screen: {
        width: window.screen.width,
        height: window.screen.height,
        availWidth: window.screen.availWidth,
        availHeight: window.screen.availHeight,
        colorDepth: window.screen.colorDepth,
        pixelDepth: window.screen.pixelDepth
      },
      viewport: {
        innerWidth: window.innerWidth,
        innerHeight: window.innerHeight,
        outerWidth: window.outerWidth,
        outerHeight: window.outerHeight,
        devicePixelRatio: window.devicePixelRatio || 1
      },
      timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || "",
      localTime: new Date().toISOString()

      // Não envio document.cookie por segurança.
      // Se precisar muito, envie apenas flags não sensíveis no backend.
    };
  }

  function getElementSummary(el) {
    if (!el) return null;

    return {
      tag: el.tagName || "",
      id: el.id || "",
      className: typeof el.className === "string" ? el.className : "",
      text: (el.innerText || el.value || el.getAttribute("aria-label") || el.getAttribute("title") || "").trim().slice(0, 200),
      href: el.href || "",
      name: el.getAttribute("name") || "",
      type: el.getAttribute("type") || "",
      role: el.getAttribute("role") || "",
      dataSection: el.dataset?.section || "",
      dataProduct: el.dataset?.product || "",
      dataPlan: el.dataset?.plan || "",
      dataService: el.dataset?.service || ""
    };
  }

  function isElementVisible(el) {
    if (!el) return false;
    const rect = el.getBoundingClientRect();

    return (
      rect.width > 0 &&
      rect.height > 0 &&
      rect.bottom > 0 &&
      rect.right > 0 &&
      rect.top < window.innerHeight &&
      rect.left < window.innerWidth
    );
  }

  function getVisibleSection() {
    const points = [
      [window.innerWidth / 2, window.innerHeight / 2],
      [window.innerWidth / 2, window.innerHeight * 0.35],
      [window.innerWidth / 2, window.innerHeight * 0.65]
    ];

    for (const [x, y] of points) {
      const el = document.elementFromPoint(x, y);
      if (!el) continue;

      const section = el.closest("section, article, main, [data-section], [data-product], [data-plan], [data-service], [id]");
      if (section) {
        const rect = section.getBoundingClientRect();

        return {
          ...getElementSummary(section),
          rect: {
            top: Math.round(rect.top),
            left: Math.round(rect.left),
            width: Math.round(rect.width),
            height: Math.round(rect.height)
          }
        };
      }
    }

    return null;
  }

  function getCurrentProductContext() {
    const selectors = [
      "[data-product]",
      "[data-plan]",
      "[data-service]"
    ];

    for (const selector of selectors) {
      const elements = Array.from(document.querySelectorAll(selector));
      const visible = elements.find(isElementVisible);

      if (visible) {
        return {
          ...getElementSummary(visible),
          product: visible.dataset?.product || "",
          plan: visible.dataset?.plan || "",
          service: visible.dataset?.service || ""
        };
      }
    }

    return null;
  }

  function getHeadingsContext() {
    return Array.from(document.querySelectorAll("h1, h2, h3"))
      .filter(isElementVisible)
      .slice(0, 5)
      .map(el => ({
        tag: el.tagName || "",
        id: el.id || "",
        text: (el.innerText || "").trim().slice(0, 150)
      }));
  }

  function getMetaContext() {
    const description = document.querySelector('meta[name="description"]')?.getAttribute("content") || "";
    const ogTitle = document.querySelector('meta[property="og:title"]')?.getAttribute("content") || "";
    const ogDescription = document.querySelector('meta[property="og:description"]')?.getAttribute("content") || "";

    return {
      description: description.slice(0, 300),
      ogTitle: ogTitle.slice(0, 300),
      ogDescription: ogDescription.slice(0, 300)
    };
  }

  function getPageContext() {
    const doc = document.documentElement;
    const body = document.body;

    const scrollHeight = Math.max(
      body?.scrollHeight || 0,
      doc?.scrollHeight || 0,
      body?.offsetHeight || 0,
      doc?.offsetHeight || 0,
      body?.clientHeight || 0,
      doc?.clientHeight || 0
    );

    const maxY = Math.max(0, scrollHeight - window.innerHeight);
    const scrollPercent = maxY > 0 ? Number(((window.scrollY / maxY) * 100).toFixed(2)) : 0;

    return {
      url: window.location.href,
      origin: window.location.origin,
      pathname: window.location.pathname,
      search: window.location.search,
      hash: window.location.hash,
      title: document.title,
      referrer: document.referrer || "",
      scroll: {
        x: window.scrollX,
        y: window.scrollY,
        maxY,
        percent: scrollPercent
      },
      viewport: {
        width: window.innerWidth,
        height: window.innerHeight
      },
      visibleSection: getVisibleSection(),
      lastClickedElement,
      currentProductContext: getCurrentProductContext(),
      visibleHeadings: getHeadingsContext(),
      meta: getMetaContext()
    };
  }

  async function collectClientContext() {
    const [micPermission, camPermission, geoPermission, notificationsPermission, mediaDevices] = await Promise.all([
      getPermissionState("microphone"),
      getPermissionState("camera"),
      getPermissionState("geolocation"),
      getPermissionState("notifications"),
      getMediaInfo()
    ]);

    let connection = null;

    try {
      const c = navigator.connection || navigator.mozConnection || navigator.webkitConnection;
      if (c) {
        connection = {
          effectiveType: c.effectiveType || "",
          rtt: c.rtt || null,
          downlink: c.downlink || null,
          saveData: c.saveData || false
        };
      }
    } catch {}

    return {
      browser: getBrowserInfo(),
      permissions: {
        microphone: micPermission,
        camera: camPermission,
        geolocation: geoPermission,
        notifications: notificationsPermission
      },
      mediaDevices,
      connection,
      pageContext: getPageContext()
    };
  }

  btnLigar.addEventListener("click", async () => {
    try {
      btnLigar.disabled = true;
      btnLigar.innerText = "⏳ Conectando...";

      setStatus("Solicitando acesso ao microfone...");
      const micOk = await prepararMicrofone();

      if (!micOk) {
        btnLigar.disabled = false;
        btnLigar.innerText = "🎤 Tentar Novamente";
        return;
      }

      setStatus("Coletando dados do navegador e da página...");
      const clientContext = await collectClientContext();
      const visitorIds = await getVisitorIds();

      setStatus("Criando chamada com wconnect.ai...");

      const payload = new URLSearchParams();
      payload.append("source", "voz.wconnect.tech");
      payload.append("leadName", "");
      payload.append("leadOrigem", "Visitante do site");
      payload.append("fluxo", "WEBRTC_SITE");
      payload.append("accountId", WCONNECT_ACCOUNT_ID);
      payload.append("inboxId", WCONNECT_INBOX_ID);

      // ID principal para deduplicação.
      payload.append("visitorId", visitorIds.deviceVisitorId);

      // ID persistente no mesmo navegador.
      payload.append("browserVisitorId", visitorIds.browserVisitorId);

      // Debug/diagnóstico para o n8n/Supabase.
      payload.append("deviceProfile", JSON.stringify(visitorIds.deviceProfile));

      payload.append("clientContext", JSON.stringify(clientContext));

      const response = await fetch("https://webhook.wconnect.tech/webhook/58e13feb-f91d-458e-8490-29a3c62c98ea-CHATWOOTV5voz", {
        method: "POST",
        body: payload
      });

      const data = await response.json();

      if (!response.ok) throw new Error(data?.error || `Erro HTTP ${response.status}`);
      if (!data.joinUrl) throw new Error("Resposta sem joinUrl");

      const { UltravoxSession } = await import("https://esm.sh/ultravox-client@0.4.2");

      setStatus("Conectando sessão de voz...");
      session = new UltravoxSession({ experimentalMessages: ["debug"] });

      await session.joinCall(data.joinUrl, "web-test");

      try { session.setOutputMedium("voice"); } catch {}
      try { session.unmuteMic(); } catch {}
      try { session.unmuteSpeaker(); } catch {}

      setStatus("✅ Conectado! Pode falar.");
      btnLigar.style.display = "none";
      btnDesligar.style.display = "block";
    } catch (error) {
      console.error("[ERRO GERAL]", error);
      setStatus("❌ Erro ao conectar: " + error.message);
      btnLigar.disabled = false;
      btnLigar.innerText = "🎤 Tentar Novamente";
    }
  });

  btnDesligar.addEventListener("click", async () => {
    try {
      if (session) await session.leaveCall();
    } catch (error) {
      console.error("[ERRO AO DESLIGAR]", error);
    } finally {
      setStatus("Chamada encerrada.");
      btnLigar.style.display = "block";
      btnDesligar.style.display = "none";
      btnLigar.disabled = false;
      btnLigar.innerText = "🎤 Converse com nosso Agente";
      session = null;
    }
  });
})();
