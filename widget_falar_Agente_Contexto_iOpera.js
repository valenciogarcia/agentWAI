...
function getScreenProfile() {
  ...
  return {
    // Usa menor/maior para reduzir diferença de orientação em tablets/celulares.
    minSide: width && height ? Math.min(width, height) : null,
    maxSide: width && height ? Math.max(width, height) : null,
...
function getBrowserFallbackId() {
  const key = "wai_visitor_id_fallback";
  let visitorId = "";
...
/**
 * visitorId determinístico provável do dispositivo.
 *
 * Objetivo:
 * - gerar UM ÚNICO visitorId;
 * - aumentar chance de reconhecer o mesmo computador mesmo trocando Chrome/Safari/Edge/Firefox;
 * - reduzir duplicidade de lead no Supabase;
 * - manter fallback antigo somente se o navegador bloquear algum recurso.
 */
async function getVisitorId() {
  try {
    const profile = {
      v: "wconnect-device-visitor-v2",

      // Mantém separado por site.
      // O mesmo computador em iopera.com.br e wconnect.tech gera IDs diferentes.
      origin: normalizeText(window.location.origin),

      // Sinais pensados para serem mais estáveis entre browsers.
      os: getApproxOS(),
  ...
  btnLigar.addEventListener("click", async () => {
    try {
      ...
      payload.append("visitorId", await getVisitorId());
